...
specification1 <- linear_reg(mixture = 0.75, penalty = tune()) %>%
  set_mode("regression") %>%
  set_engine("glmnet")
...
penalty_grid <- grid_regular(penalty(range = c(-10, 10)), levels = 30)
penalty_grid
...
write.csv(test_hogares_cargue,"C:/Users/afdia/OneDrive - Universidad de los Andes/Maestría en Economía Aplicada/Big Data y Machine Learning/Repositorios-GitHub/Taller-3/stores/Resultados/Predicción Ingreso/Elastic Net/ElasticNet_02.csv", row.names = FALSE)